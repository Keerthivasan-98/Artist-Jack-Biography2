  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        >
          <div className="p-1 px-2 border border-yellow-500/20 bg-yellow-500/5 rounded flex items-center justify-center">
            <span className="text-[10px] font-mono tracking-widest text-yellow-500/80 font-bold">AJ</span>
          </div>
          <div className="flex flex-col">
            <span className="text-[9px] font-mono tracking-widest text-[#D4AF37]">{t('hero.artist')}</span>
            <span className="text-lg font-serif font-bold tracking-tight text-white group-hover:text-[#D4AF37] transition-all">
        ...
        <div className="hidden lg:flex items-center gap-1 bg-[#121212]/30 border border-white/5 p-1 rounded-full px-2 backdrop-blur-sm">
          {navItems.map((item, i) => (
            <motion.a
              ...
              className={`relative px-4 py-2 rounded-full text-xs font-mono tracking-wider transition-colors duration-300 ${
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          <motion.a
            ...
            className="px-4 py-1.5 border border-yellow-500/30 rounded-md bg-[#D4AF37]/5 text-xs text-[#D4AF37] font-semibold tracking-wider hover:bg-[#D4AF37] hover:text-black transition-all duration-300 shadow-[0_0_15px_rgba(212,175,55,0.1)] hover:shadow-[0_0_20px_rgba(212,175,55,0.3)]"
            initial={{ opacity: 0, x: 25 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8 }}
          >
            ...
            <div className="flex flex-col space-y-3 my-auto text-center py-6">
              {navItems.map((item, index) => (
                <motion.a
                  ...
                  href={`#${item.id}`}
                  onClick={(e) => handleNavClick(e, item.id)}
                  className={`text-xl font-serif tracking-wide py-1 border-b border-white/5 transition-all block ${
                    activeSection === item.id
                ...
                {LANGUAGES.map((lang) => {
                  ...
                  return (
                    <button
                      ...
                      className={`py-1.5 text-[11px] font-mono rounded-lg border transition-all ${
                        isSelected
            ...
            <div className="border-t border-white/5 pt-6 flex flex-col space-y-2 items-center text-center">
              ...
              <div className="flex gap-4">
                <a
                  ...
                  className="text-xs text-[#D4AF37] border-b border-[#D4AF37]/20"
                >
                  Instagram
                </a>
                <a
